<x-admin-layout title="Tambah Kategori Presenter">
    <div class="mb-6">
        <a href="{{ route('master.presenter-categories.index') }}" class="text-sm text-indigo-600 hover:text-indigo-800">← Kembali</a>
        <h2 class="mt-2 text-xl font-semibold text-slate-900">Tambah Kategori Presenter</h2>
    </div>

    <x-card>
        <form method="POST" action="{{ route('master.presenter-categories.store') }}">
            @csrf
            @include('master.presenter-categories._form')
